...
vol2d = function(pfsc, r)
{
     for(i in 1L:2L)
     {
      pfsc[,i] = (pfsc[,i]-min(pfsc[,i]))/(max(pfsc[,i])-min(pfsc[,i]))
     }
...
canonrsm.v3 = function(statpt,ystat,eigratio,evecang,ngrid) {          
...
ui = (fluidPage(
   ...
   titlePanel("Multiple Optimal Design: Maximum VS Maximum"),
    ...
    sidebarPanel(
      ...
      sliderInput("Y1", "Maximum value of Y1:", min = 0, max = 200, value = 100, step = 1),
      ...
      sliderInput("Y2", "Maximum value of Y2:", min = 0, max = 200, value = 50, step = 1),
...
output$plot = renderPlot({
       ...
       temp.r1 = canonrsm.v3(c(input$M1X1,input$M1X2),input$Y1,input$er1,input$M1Ea,20) 
       temp.r2 = canonrsm.v3(c(input$M2X1,input$M2X2),input$Y2,input$er2,input$M2Ea,20)

       cdat1 = as.data.frame(temp.r1[[2]])
       cdat2 = as.data.frame(temp.r2[[2]])
       names(cdat1) = c("X1", "X2", "Y1")
       names(cdat2) = c("X1", "X2", "Y2")
    ...
    cont2 = ggplot(cdat2, aes(X1, X2, z = Y2))+
...
# Get data to plot
     ...
     pdes = ggplot(pdat,aes(x=X1, y=X2))+
            ...
            geom_point(shape = 19, colour = 'red', size = (pdat$Y1 - min(pdat$Y1))*5/(max(pdat$Y1)-min(pdat$Y1))+1)+
            scale_x_continuous(limits = c(-1, 1), breaks=seq(-1, 1, by = 0.2))+
            scale_y_continuous(limits = c(-1, 1), breaks=seq(-1, 1, by = 0.1))

      ppf = ggplot(pdat,aes(x=Y1, y=Y2))+
            theme(axis.text = element_text(colour = 'black', size=16),
                  axis.title = element_text(size = 16), legend.position="none")+
            geom_step(direction = "vh", size = 1, colour = 'blue')+      
            geom_point(shape = 19, colour = 'red', size=(pdat$Y1 - min(pdat$Y1))*5/(max(pdat$Y1)-min(pdat$Y1))+1)+
            geom_point(x = max(pdat$Y1), y = max(pdat$Y2), shape = 42, size = 16)+
            geom_text(aes(x = min(pdat$Y1)+diff(range(pdat$Y1))/5, 
                          y = min(pdat$Y2)+diff(range(pdat$Y2))/6, 
                          label=paste("Hypervolume: ", volume)), size = 6)
