...
vol2d = function(pfsc, r)
{
	...
	{ pfsc[,1] = (pfsc[,1]-min(pfsc[,1]))/(max(pfsc[,1])-min(pfsc[,1]))
...
ui = (fluidPage(
   ...
   titlePanel("Multiple Optimal Design: Maximum VS Target"),
    ...
    sidebarPanel(
      ...
      sliderInput("Y1", "Maximum value of Y1:", min = 0, max = 250, value = 150, step = 1),
      ...
      sliderInput("Y2", "Maximum value of Y2:", min = 0, max = 250, value = 100, step = 1),
      sliderInput("YT", "Target value of Y2:", min = 0, max = 250, value = 80, step = 1),
...
output$plot = renderPlot({
       ...
       temp.r1 = canonrsm.v3(c(input$M1X1,input$M1X2),input$Y1,input$er1,input$M1Ea,20, 0) 
       temp.r2 = canonrsm.v3(c(input$M2X1,input$M2X2),input$Y2,input$er2,input$M2Ea,20, input$YT)

       cdat1 = as.data.frame(temp.r1[[2]])
       cdat2 = as.data.frame(temp.r2[[2]])
       names(cdat1) = c("X1", "X2", "Y1")
       names(cdat2) = c("X1", "X2", "Dist")

    cont1 = ggplot(cdat1, aes(X1, X2, z = Y1))+
    ...
    cont2 = ggplot(cdat2, aes(X1, X2, z = Dist))+
...
# Get data to plot
     ...
     pdes = ggplot(pdat,aes(x=X1, y=X2))+
            ...
            geom_point(shape = 19, colour = 'red', size = (pdat$Y1 - min(pdat$Y1))*5/(max(pdat$Y1)-min(pdat$Y1)+1)+1)+
            scale_x_continuous(limits = c(-1, 1), breaks=seq(-1, 1, by = 0.2))+
            scale_y_continuous(limits = c(-1, 1), breaks=seq(-1, 1, by = 0.1))

      ppf = ggplot(pdat,aes(x=Y1, y=Dist))+
            theme(axis.text = element_text(colour = 'black', size=16),
                  axis.title = element_text(size = 16), legend.position="none")+
            geom_step(color='blue', direction = "vh", size = 1)+
            geom_point(shape = 19, colour = 'red', size = (pdat$Y1 - min(pdat$Y1))*5/(max(pdat$Y1)-min(pdat$Y1)+1)+1)+
            geom_point(x = max(pdat$Y1), y = 0, shape = 42, size = 16)+
            scale_y_continuous(limits = c(0, max(pdat$Dist)))+
            geom_text(aes(x = min(pdat$Y1)+diff(range(pdat$Y1))/3, 
                          y = max(pdat$Dist)-diff(range(pdat$Dist))/5, 
